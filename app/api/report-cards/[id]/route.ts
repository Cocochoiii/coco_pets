...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (auth.user.role !== 'admin' && auth.user.role !== 'staff') {
      const bookingId = report.booking?._id || report.booking
      ...
      // Mark as viewed
      await ReportCard.findByIdAndUpdate(id, { viewedAt: new Date(), viewedBy: auth.userId })
    }
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const updated = await ReportCard.findByIdAndUpdate(id, { $set: updates }, { new: true })
