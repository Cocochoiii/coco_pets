...
// Define the service type with optional priceRange
interface ServiceInfo {
  name: string
  icon: typeof Cat | typeof Dog | typeof Sun
  price: number
  priceRange?: string
  unit: string
  description: string
...
export default function BookingPage() {
  ...
  const updatePet = (index: number, field: keyof PetInfo, value: string | boolean) => {
    const updated = [...pets]
              ...
              {step === 1 && (
                  <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      {(Object.entries(SERVICES) as [ServiceType, ServiceInfo][]).map(([key, service]) => {
